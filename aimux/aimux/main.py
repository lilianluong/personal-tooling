...
def main() -> None:
    ...
    parser.add_argument(
        "--codex",
        action="store_true",
        help="Start in codex mode: new sessions launch codex instead of claude",
    )
    ...
    sp.add_argument("--prompt", default=None, help="Initial prompt to send to the agent")
    sp.add_argument("--codex", action="store_true", help="Launch codex instead of claude")
    ...
    if args.command == "spawn":
        ...
        spawn_session(args.workspace, args.name, args.prompt, agent="codex" if args.codex else "claude")
        print(args.name)
        ...
        AimuxApp(agent="codex" if args.codex else "claude").run()
    else:
        parser.print_help()
        sys.exit(1)
